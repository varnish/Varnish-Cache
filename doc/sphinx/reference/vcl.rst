...
It will use the value of ``req.hash``,
just as the normal cache lookup methods.
...
larger the subnet, the more overhead.
...
``.window``
  ...
  Defaults to 8.
``.threshold``
  How many of the polls in ``.window`` should have succeeded for us to
...
``vcl_pass``
  ...
  ``restart``
    ...
    of restarts is higher than ``max_restarts``,
...
``vcl_hit``
  ...
  ``restart``
    Restart the transaction. Increases the restart counter. If the number 
    of restarts is higher than ``max_restarts``,
    varnish emits a guru meditation 
    error.
...
``req.hash_ignore_busy``
  ...
  this if you have two servers looking up content from each other to 
...
``beresp.do_stream``
  ...
  stored in memory. As of Varnish Cache 3.0, the object will be marked as busy
  as it is delivered so only the client can access the object.
...
and Per Buer. Jean Jordaan tidied the markup.
